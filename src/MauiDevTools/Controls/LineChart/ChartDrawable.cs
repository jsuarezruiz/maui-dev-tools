...
{
    public abstract class ChartDrawable : IDrawable, IDisposable
    {
        #region Public Properties

        bool _maxAxisLabelWidthDetermined = false;
        ...
        float _axisFontSize = 11.0f;
        float _footerLabelTextSize = 10.0f;
        Color _strokeColor = Colors.Black;
        Color _fillColor = Colors.Transparent;
        Color _fontColor = Colors.Black;
        Color _footerLabelsFontColor = Colors.Black;
        float _margin = 15f;
        float _footerLabelsMargin = 8f;
        float _itemSeparationMargin = 8f;
        float _headerValuesMargin = 20f;
        bool _enableAntialias = true;
        IFont _font = Microsoft.Maui.Graphics.Font.Default;
        float _strokeSize = 2.5f;
        protected bool _displayValueLabelsOnTop = true;
        bool _isLabelTextTruncationEnabled = true;
        ObservableCollection<ChartItem> _entries;

        bool _displayVerticalAxisLines = false;
        bool _displayHorizontalAxisLines = false;
        float _pathsColorOpacity = 0.6f;

        float _axisLinesStrokeSize = 0.9f;
        Color _axisLineColor = Colors.LightGray;
        float[] _axisDashPattern = new float[] { 6, 6 };
